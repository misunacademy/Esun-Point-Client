            ...
            <div className="p-4 space-y-2">
                ...
                {isOpen && (
                    <div className="mt-3 relative group">
                        ...
                        <div
                            className="relative p-[1.5px] rounded-xl overflow-hidden transition-all duration-500 group-hover:shadow-[0_0_24px_6px_hsl(217_91%_60%/0.45)] shadow-[0_0_14px_2px_hsl(217_91%_60%/0.3)]"
                        >
                            {/* Spinning border */}
                            <div
...
function CheckoutContent() {
    ...
    const currentBatch = (enCurrentRes?.data as any) ?? (enUpcomingRes?.data as any)?.[0];
    ...
    const currentCourse = (enCourseData?.data as any);
    const courseFee = currentBatch?.price ?? currentCourse?.price ?? 3000;
    const courseTitle = currentCourse?.name ?? 'MISUN Academy Course Enrollment';

    const allLoading = enCourseLoading || (!!enCourseId && enCurrentLoading) || (!!enCourseId && !enCurrentRes?.data && enUpcomingLoading);
                ...
                <DialogContent className="sm:max-w-lg font-bangla bg-[#060f0a] border border-primary/20 text-white z-50 mt-5">
                    <DialogHeader className="space-y-4">
                        ...
                        <DialogDescription className="text-center text-white/50">
                            See the enrollment schedule for all courses below.
                        </DialogDescription>
                    </DialogHeader>

                    <div className="py-4 space-y-3">
                        <CourseEnrollmentCard
                            courseData={enCourseData?.data}
